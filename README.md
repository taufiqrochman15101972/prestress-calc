...
**🪨 Pondasi (Statik & Dinamik)** — fondasi dalam & dangkal (Bowles/Budhu/Das/TM 5-818-1, books 194–205): kapasitas aksial tiang/bore-pile/shaft (α/β/Meyerhof, Q_s+Q_p), grup (Converse-Labarre + blok), penurunan Vesic, lateral Broms, pemancangan dinamik (ENR/Hiley/Janbu), daya dukung dangkal Vesic, fondasi mesin half-space (Richart) + SSI (Veletsos). **Centang opt-in** `🪨 Sertakan analisis & desain pondasi` di panel input → §30 Pondasi ikut dihitung & muncul di laporan PDF (3-baris); jika tidak dicentang, tidak ikut. **🧮 FEM Modeler (gaya STAAD.Pro)** — ekosistem FEM/FEA 3-lapis: **Pre-processor** (tabel node/member + 3 cara copy/paste: linear/mirror/rotate), **Solver Core** (elemen balok-kolom 2D aksial+lentur+geser Timoshenko bebas shear-locking; solver Float64Array zero-copy; perakitan global, BC, LU), **Post-processor** (lendutan, diagram N/V/M, reaksi). Pustaka elemen juga memuat **flat-shell Q4 Mindlin-SRI** (membran + pelat, **bebas shear locking**). Divalidasi vs rumus tertutup. Fondasi TypeScript (zero-copy), target Phase-2 native Python+Julia+Zig. Sumbu global **X→kanan, Y→depan, Z→atas** (isometrik). **🧊 Rangka Ruang 3D** — elemen balok-kolom 3D 6 DOF/node (aksial+torsi+lentur 2-sumbu), modeler isometrik + gaya batang. **🎚 Kompatibilitas Regangan ULS** — analisis lentur ultimit berlapis (Naaman), f_ps dari kurva σ–ε aktual, prategang penuh & sebagian. Backend solver **swappable** (`fem/backend.ts`) → siap diganti native Julia/Zig zero-copy tanpa ubah UI. **📉 Garis Pengaruh & Beban Bergerak** — analisis influence-line & moving-load gaya MIDAS/Civil (beban satuan ditelusuri lewat solver FEM → garis pengaruh R₀/M_mid/V_mid, kendaraan gandar → amplop maks/min). GUI input gaya STAAD.Pro; solver/output/post-processing diarahkan ke gaya MIDAS & Robot. **Cek desain otomatis** (AISC/SNI 1729 interaksi H1-1, rasio utilisasi berwarna) + **P-Δ nonlinier geometrik** (amplifikasi 1/(1−P/Pcr)) langsung di modeler 🧮. **🌊 Time-History Dinamik** — integrasi Newmark-β (gempa/harmonik/pulsa, u(t), DAF, resonansi). **📈 Pushover** — kurva kapasitas sendi-plastis (event-to-event) sampai mekanisme. **🛡 Isolasi Dasar & Damper** — desain isolasi seismik (AASHTO/SNI). **🧵 Fiber Moment-Curvature (UMAT)** — M–φ nonlinier metode serat (Hognestad + baja elastik-plastis, Newton-Raphson). **▣ Shell 3D Penuh** — rakit-solve flat-shell 6-DOF/node (membran+lentur+drilling). **⛰ Stabilitas Lereng** — FS lereng tak-hingga + busur lingkaran (Bishop/Fellenius). **◫ Tulangan Shell** — desain tulangan beton shell metode sandwich (IASS) dari 8 resultan tegangan. Solver backend **iteratif CG** (`fem/sparsebackend.ts`) lewat seam yang sama (native-ready). Semua solver **diverifikasi vs benchmark teori** (balok jepit-jepit, kantilever-prop, menerus, buckling Euler Gere-Timoshenko, eigenvalue 2-DOF Greenwood — MIDAS Verification Manual). **▦ Pelat/Shell FEM** — meshing rektangular + solve elemen Q4 Mindlin-SRI (bebas shear-locking), tepi SS/clamped, tekanan merata → medan lendutan w(x,y), permukaan isometrik berwarna, validasi vs teori pelat tipis. Tab **📊** kini punya toggle **Solver FEM ⇄ Closed-form**. **📊 Diagram Gaya Dalam & Tegangan** — visual real-time bergaya OriginPro / IDEA StatiCa / Robot / MIDAS: momen utama M_z, momen lateral M_y, geser V_x/V_y, aksial N (tarik+/tekan−), torsi T_x, lendutan ±Z & ±Y — kurva terisi gradien warna jet, toggle centang real-time. Klik di bentang (0–L) → gaya dalam di titik itu; klik tinggi penampang (−yb…+yt) → tegangan σ (Navier & kernel, ekuivalen) + lendutan, dengan colormap (biru tekan→merah tarik). Berbasis kesetimbangan mekanika bahan lanjut, disiapkan untuk FEM/FEA. **📐 Impor Gambar DWG/DXF** — **konverter DWG→DXF terpasang** (LibreDWG WebAssembly): unggah `.dwg` langsung (otomatis dikonversi) atau `.dxf` → parser mengekstrak extents (panjang/lebar jembatan), bounding-box & tinggi profil girder, spasi girder/diafragma, nilai DIMENSION & teks, kotak abutment/pier/pilecap/pierhead → tombol "terapkan" mengisi bentang/lebar dek/tinggi girder. **🌋 Dinamik & Gempa Bangunan Bawah** — analisis dinamik & desain gempa substruktur (AASHTO Guide Spec Seismic / Caltrans SDC / Priestley DBD / SNI 2833 / Seed–Idriss): respons SDOF (T, Sd, V_base), modal 2-DOF SRSS, desain kapasitas pilar (M_po overstrength, L_p sendi plastis, μ_Δ daktilitas, P-Δ), likuifaksi (CSR/CRR/MSF/FS). Ikut terhitung & muncul di **§31 laporan PDF** saat centang Pondasi aktif. **🌎 Beban & Gempa SNI** — SNI 2833:2016 spektrum respons gempa jembatan (As/S_DS/S_D1/C_sm/zona/R) + SNI 1725:2016 beban sekunder (angin EWs/EWl, rem TB, suhu EUn) (books 207/211). **🪢 Jembatan Kabel** — cable-stayed Gimsing (fan/harp, gaya stay V/sinθ, Ernst E_eff, aksial pilon) (book 209). **🔺 Jembatan Rangka Baja** — rangka Pratt/Warren/Howe (chord M/h, diagonal V/sinθ, tarik leleh + tekan tekuk F_cr; Rochman & Suhariyanto + SNI 1729) (book 210).
...
npm test           # vitest (169 assertion, termasuk verifikasi benchmark)
