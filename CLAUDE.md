...
- File rujukan tetap di folder ini: `PRD Prategang.md` (spesifikasi mengikat, riwayat revisi), `CLAUDE.md` (ini), `.claude/skills.md` (definisi skill per engine), `.claude/agents.md`, PDF buku referensi bernomor `1.pdf`–`250.pdf` (+ `ST1`–`ST3` STAAD.Pro manual, `MD (1)`–`MD (429)` (+ `MD (40/71/133).chm`, `MD (*).jpg/.gif` gambar) MIDAS technical refs/Gen+Civil tutorials + **MIDAS FEA Verification Manual** (MD174+: Linear Buckling, Eigenvalue, Concrete Crack, CFD, Fatigue, dll), `gambar ALLPLAN.xlsx` acuan tampilan output, `170.xls`, `174.jpg`, `123.ppm`; + `MD (430)`–`MD (522)` (incl. MIDAS GTS/DIANA geoteknik 470–522), `SP (1)`–`SP (12)` (CSiBridge), `CSiBridge Enhancements…BRIDGE ANALYSIS DESIGN AND RATING.html`, + `251`–`255.pdf` (251 CSiBridge, 253 IASS shell-reinforcement, 254 offshore-wind foundations, 255 MIDAS GTS USSR/UMAT user-material); **buku 1–255 + ST1–3 + MD1–522 + SP1–12 tuntas**) + **54 file `*.dwg`** (gambar pelaksanaan/shop-drawing AutoCAD: seri 01–52 + LIST + `154-PCI GIRDER LAMA` + `01-kaltim` — biner AC1015/AC1018; kini DAPAT dibaca via konverter DWG→DXF terpasang, lihat di bawah) + **`O1.pdf`–`O10.pdf`** (acuan GAYA tampilan output OriginPro: 3D surface/contour multicolor, colormap jet — dasar visual tab 📊 Diagram Gaya Dalam). **Rujukan dasar GAMBAR output desain** = `A.pdf` (SD APB Girder Depth-End L=40.8m), `B.pdf` (DED NPEA PCI-Girder H=2.10m), `C.pdf` (DED PCI-Girder Manggarai H=1.25m) + semua file gambar (`*.png`) sebelumnya — gaya DED: Tampak Samping (elevasi+tendon+blok ujung+perletakan), Potongan berdimensi lengkap, blok CATATAN/NOTES material, kop gambar; **buku 153** = "Perhitungan Teknis PCI Girder Standar PT Adhi Persada Beton" (H=2.1m, L=40.8m, fc'=50MPa) sebagai tambahan dasar prosedur (beban MS/MA/TD/TT/TB/EW/EQ/EUn, kehilangan ES/R/CR/SH/F/A, tegangan, lendutan, geser) — semua sudah tercakup engine eksisting.
...
- **Konverter DWG→DXF + Diagram Gaya Dalam OriginPro-style** (sesi lanjutan): (1) **Konverter DWG→DXF dalam proyek & reusable** — `lib/dwgConvert.ts` me-lazy-load **LibreDWG WebAssembly** (`@mlightcad/libredwg-web`, dynamic import client-only sehingga tak membengkakkan bundle) → `dwg_write_dxf(ArrayBuffer)` menghasilkan teks DXF → `engine/dxfimport.ts`. Tab 📐 kini menerima `.dwg` LANGSUNG (otomatis dikonversi) maupun `.dxf`. (2) **Tab 📊 Diagram Gaya Dalam & Tegangan** (`engine/internalforces.ts` + `ForceDiagramsCalculator.tsx`) — visual real-time gaya OriginPro/IDEA-StatiCa/Robot/MIDAS dari kesetimbangan mekanika-bahan (pre-FEM, disiapkan untuk FEM/FEA): medan M_z (lentur utama), M_y (lateral), V_y, V_x, N (tarik+/tekan−), T_x (puntir), lendutan Δz (−turun/+naik) & Δy, semua dengan kurva terisi **gradien warna jet** dan toggle centang real-time. **Klik di bentang (0–L)** → gaya dalam pada titik; **klik tinggi penampang (−yb…+yt)** → tegangan σ DUA rumus ekuivalen (Navier N/A+P·e·y/I−M·y/I & kernel (−P/A)(1−e·y/r²)−M·y/I, nilainya sama) + lendutan, dengan colormap σ (biru tekan→merah tarik) + colorbar. Membaca model hidup dari store (Pe, e, EI, I_y). Acuan gaya = `O1.pdf`–`O10.pdf`.
- **Verifikasi geoteknik (konsolidasi + Mohr-Coulomb) + MD470–522/251–254 (aturan benchmark mutlak)** (sesi lanjutan): tinjau ulang MD(470)–(522)+MD(470).chm (MIDAS GTS/DIANA geoteknik & terowongan: tutorial, release notes, riset RELUIS, verifikasi konstitutif) + 251 (CSiBridge, tercakup), 252 (scan kosong), 253 (IASS shell-reinf → `shellreinf.ts`), 254 (offshore-wind → parsial pile lateral). Konten verifikasi MD(474) (triaksial MMC/Mohr-Coulomb) & MD(470) (konsolidasi) ditutup sbg benchmark BERANGKA-MUTLAK: **`engine/consolidation.ts`** (Terzaghi 1-D: deret eksak U(Tv), invers Tv(U), settlement S_c=(Cc/(1+e0))·H·log10((σ0+Δσ)/σ0), S(t)=U·S_c, t50/t90) + **`engine/mohrcoulomb.ts`** (Kp=tan²(45+φ/2), triaksial saat runtuh σ1f=σ3·Kp+2c√Kp, q_f, τ_f). Diverifikasi vs angka teks-baku ABSOLUT (`tests/geotech_verif.test.ts`, 9 test): U=50%↔Tv=0,197; U=90%↔Tv=0,848; U(0,2)≈0,504; c=0/φ=30/σ3=100→q_f=200 kPa; Tresca q_f=2cu; faktor daya-dukung Nc(φ=0)=5,14, Nq(30)=18,40, Nc(30)=30,14, Nγ-Vesic(30)=22,40 (kode Vesic eksisting). UI: blok ringkas di tab ⛰ (konsolidasi + triaksial real-time). 183 test hijau, build OK.
- **Backend native Julia/Zig SUNGGUHAN** (sesi lanjutan): sumber native nyata & dapat-dikompilasi di `native/` di belakang seam `SolverBackend` yang SAMA — (1) `native/zig/sparse_solver.zig` (C-ABI `solve_cg(K,n,f,x,scratch,max_it,tol)` = PCG Jacobi identik dgn `sparsebackend.ts`, bebas-alokasi → dapat dibangun jadi shared-lib `.dll/.so/.dylib` LEWAT `zig build` MAUPUN `femsolver.wasm` freestanding lewat `zig build wasm`; `build.zig`), (2) `native/julia/FemSolver.jl` (`Base.@ccallable solve_cg` tanda-tangan identik, `unsafe_wrap` zero-copy, `build_lib.jl` via PackageCompiler → shared-lib), (3) **jembatan FFI Node** `engine/fem/nativebackend.ts` (`tryActivateNativeBackend()`/`loadNativeBackend()` memuat lib via `koffi`, register lewat `setSolverBackend`; **fail-soft** → kalau toolchain/lib/koffi absen kembalikan `{ok:false}` & tetap pakai backend TS CG yang hasilnya identik), (4) `native/build.sh`+`build.ps1`+`README.md`. **Kejujuran lingkungan:** env ini TIDAK punya Zig/Julia/gcc/clang/cargo (diverifikasi) & Vercel serverless tak bisa load native `.so/.dll` → lib TIDAK dibangun/di-deploy di sini; sumbernya asli & siap-bangun di mesin ber-toolchain (Node lokal/self-host). 2 test fallback baru (loadNativeBackend→null, tryActivate→{ok:false} tanpa ubah backend aktif). 174 test hijau, build OK. `require` tak-langsung (`eval("require")`) menjaga FFI keluar dari bundle web.
- **Mode/alur kerja desain girder** (sesi lanjutan): toggle di InputPanel (Mode Desain) — **Alur Kerja**: `DIRECT` (langsung desain dari bentang/gaya dalam, TANPA analisis struktur) ⇄ `ANALYSIS_FIRST` (mulai dari FEM lalu desain); **Lingkup**: `FULL` (desain lengkap SLS+ULS+geser+lendutan+detailing) ⇄ `STRESS_ONLY` (cek tegangan SLS saja → laporan menyembunyikan §9/§10 ULS). AppSettings.workflowMode/designScope (persist). **Input gaya dalam langsung**: `loads.directMoments{enabled,Mg,Msdl,Mlive}` (kN·m) — runPipeline memakai M ini menggantikan qL²/8 (untuk yang sudah punya momen dari analisis sendiri). FEM Modeler 🧮 punya tombol **“→ kirim gaya ke desain girder”** (M maks batang → directMoments.Mlive, mengaktifkan input langsung) = jembatan ANALYSIS_FIRST → desain. Laporan PDF & store menghormati scope/sumber-momen.
- **Stabilitas lereng + tulangan shell + buku MD470–522/251–254** (sesi lanjutan): buku MD(470)–(522) = MIDAS GTS/DIANA **geoteknik & terowongan** (model konstitutif tanah, construction-stage embankment/tunnel/abutment, slope stability, anchor-soil, pile-group, teori elemen). DUA gap nyata ditutup: (1) **Stabilitas lereng** `engine/slopestability.ts` — lereng tak-hingga translasi (FS=[c+γz cos²β tanφ]/[γz sinβ cosβ], ±seepage) + busur lingkaran **metode irisan Bishop simplified (iteratif) & Fellenius** (irisan otomatis lereng seragam, ru) + tab **⛰** (sketsa lereng+lingkaran); verifikasi FS=tanφ/tanβ. (2) **Tulangan shell** `engine/shellreinf.ts` (file 253 IASS Medwadowski-Samartin) — metode SANDWICH: 8 resultan (n_x/n_y/n_xy + m_x/m_y/m_xy) → 2 lapis baja di lengan z=t−2c, aturan Baumann/CEB As·fy=n+|n_xy| → As_x/As_y tiap muka; tab **◫**; mengambil resultan dari solver shell ▣. File 251=CSiBridge (tercakup), 254=offshore-wind foundation (parsial via pile lateral). (3) **UMAT user-material** `engine/umat.ts` (file 255 MIDAS GTS USSR / ABAQUS UMAT) — antarmuka 1D σ(ε)+tangen E_t: umatLinear, umatHognestad (beton nonlinear-elastik), umatElastoPlastic (baja bilinear + hardening) + computeUmatCurve; tab **⚗** (uji kurva σ–ε), dapat dicolok ke serat M–φ 🧵 & truss/fiber-frame — memperkaya nonlinier material. Angka PDF bukan acuan.
- **Solver backend CG + shell 3D penuh + fiber UMAT + buku MD430–469/SP1–12/CSiBridge** (sesi lanjutan): **#1 Backend solver iteratif** `engine/fem/sparsebackend.ts` (Preconditioned Conjugate Gradient + Jacobi, algoritma SPD yang dipakai backend native Julia/Zig di atas CSR; lewat seam `setSolverBackend`/`solve` yang SAMA → swap tanpa ubah elemen/UI; diuji ≡ dense-LU). **#2 Shell 3D PENUH** `engine/fem/shellsolver.ts` (rakit `flatShellK` 24×24 6-DOF/node = membran + pelat Mindlin-SRI + drilling, BC tepi + tekanan + tarik tepi, solve) + tab **▣**; divalidasi vs teori pelat (w≈α·q·a⁴/D) & membran (u≈N·a/EA). **#3 Nonlinier material UMAT** `engine/fibermomentcurvature.ts` (metode SERAT: hukum Hognestad beton + crushing & baja elastik-plastis, Newton-Raphson ΣF=N tiap kurvatur → kurva M–φ retak/leleh/ultimit, daktilitas) + tab **🧵**; divalidasi M_u≈As·fy·(d−a/2). Buku MD(430)–(469) = tutorial/manual MIDAS Civil construction-stage & teori (Analysis Manual/Algorithm, release notes, tesis) — tercakup. SP(1)–(12) + CSiBridge HTML = CSiBridge (moving-load influence, multi-code load rating, tendon layout, construction-stage creep/shrinkage time-dependent, nonlinear hinge/P-Δ/large-displacement, substructure column design, shear+torsi) — SEMUA fitur utama sudah terwakili (bridgeload/influence, rating, tendon, creepshrinkage/segmental, pushover/pdelta, substructure, torsion/mcft); memvalidasi arah, tanpa kode redundan. Tidak ada file benchmark baru (MIDAS Verification Manual = MD174–213, sudah diuji). Backend native Julia/Zig tetap butuh toolchain (didokumentasi). Angka PDF bukan acuan.
- **Pushover + isolasi-dasar + benchmark verification (MIDAS) + buku MD71–429** (sesi lanjutan): **Pushover** `engine/fem/pushover.ts` (event-to-event sendi plastis, rilis momen via kondensasi statik, kurva kapasitas base-shear–perpindahan sampai mekanisme) + tab **📈**; verifikasi kantilever V_max≈M_p/H. **Isolasi dasar & damper** `engine/baseisolation.ts` (AASHTO Guide Spec Isolation/SNI: T_iso, faktor redaman B=(ζ/0,05)^0,3, reduksi geser dasar, perpindahan isolator) + tab **🛡**. **Perbaikan P-Δ**: amplifikasi diukur per-arah translasi (aksial≈1, lateral teramplifikasi) → 1/(1−P/Pcr) benar. **VERIFIKASI BENCHMARK** (`tests/benchmark.test.ts`) terhadap teori: balok jepit-jepit UDL δ=wL⁴/384EI & M=wL²/12, kantilever-prop reaksi 3wL/8, menerus 2-bentang reaksi tengah 1,25wL, **Linear Buckling-B1 Case 1 P_cr=π²EI/L² & Case 2 π²EI/4L² (Gere & Timoshenko)**, **Eigenvalue 2-DOF golden-ratio (Greenwood, MD191)**. Buku **MD (71)–(132)** = tutorial MIDAS Civil construction-stage (MSS/FCM/ILM/cable-stayed/suspension/arch, creep-shrinkage time-dependent, unknown-load-factor, moving load, fiber/nonlinear-link) — tercakup segmental.ts/creepshrinkage.ts/cablestayed.ts; construction-stage sistematik = next. Buku **MD (133)–(429)** = MIDAS FEA Verification Manual + tutorial (kasus benchmark Linear Buckling/Eigenvalue/Concrete-Crack/CFD/Fatigue/Solid) — kasus yang dapat direproduksi engine (buckling, eigenvalue, statik) diuji vs nilai teori + referensi; kasus elemen lanjut (solid/CFD/total-strain-crack) = referensi untuk pengembangan elemen berikutnya. Gambar `MD (*).jpg/.gif` = acuan tampilan. .chm biner = referensi. Angka PDF bukan acuan kode kecuali nilai-target benchmark teori yang memang untuk verifikasi.
- **Design-check + nonlinier P-Δ + time-history (gaya MIDAS/Robot) + buku MD40–70** (sesi lanjutan): **#1 Cek desain otomatis** `engine/fem/designcheck.ts` (baja AISC 360/SNI 1729: tekuk lentur Fcr, φPn tarik/tekan, φMn, φVn, interaksi H1-1, **rasio utilisasi** demand/kapasitas) — terintegrasi di tab 🧮 (kolom rasio per batang berwarna hijau/kuning/merah + rasio maks, gaya batang dari solver FEM, gaya MIDAS/Robot "design result"). **#2a Nonlinier geometrik P-Δ** `engine/fem/pdelta.ts` (kekakuan geometrik konsisten K_g(P), iterasi solve→axial→K_g→re-solve, amplifikasi δ₂/δ₁→1/(1−P/Pcr), deteksi tekuk) — toggle di tab 🧮. **#2b Time-history** `engine/timehistory.ts` (Newmark-β rata-rata percepatan γ=½ β=¼ stabil tanpa syarat, SDOF gempa-sinus/harmonik/pulsa → u(t), puncak, DAF, resonansi 1/2ζ) + tab 🌊. Buku **MD (40)–(70)** = tutorial/guide MIDAS Gen (desain baja & Eurocode RC, **pushover** MD55, **base-isolation/damper** MD60, **linear time-history** MD70, release notes) + MD(40).chm (help terkompilasi, biner) — memvalidasi arah MIDAS; pushover & base-isolation = peningkatan MIDAS berikutnya di atas seam solver yang sama. Divalidasi (cek rasio, P-Δ vs 1/(1−P/Pcr), Newmark Tn/DAF/resonansi). Angka PDF bukan acuan.
- **Garis pengaruh / moving-load (MIDAS-style) + buku MD1–39/ST1–3/ALLPLAN** (sesi lanjutan): arah upgrade — **GUI input (node/elemen/copy) gaya STAAD.Pro** (sudah: tab 🧮/🧊 modeler + 3 cara copy), **solver/output/post-processing/design gaya MIDAS & Robot**. Buku **MD (1)–(39)** = referensi teknis MIDAS (formulasi elemen plane stress/strain/axisymmetric/plate, analisis nonlinier material/geometri & inelastic time-history, **influence surface/moving load** MD-1, buckling LTB, flowchart desain kolom/baja); `gambar ALLPLAN.xlsx` = acuan tampilan output. Gap nyata pertama yang ditutup = **garis pengaruh & beban bergerak** `engine/fem/influence.ts` + tab **📉** (beban satuan ditelusuri lewat solver FEM kita → garis pengaruh R₀/M_mid/V_mid; kendaraan gandar digeser → amplop maks/min + posisi kritis; bentang sederhana/menerus) — setara fitur moving-load MIDAS/Civil. Sekaligus **recovery reaksi diperbaiki** di `frame.ts` → R = K_pure·d − F (eksak walau beban tepat di tumpuan; bukan lagi penalti). Semua viewport memakai sumbu global X→kanan/Y→depan/Z→atas isometrik (konfirmasi). Sisa MD (flowchart desain kolom/baja, nonlinier/time-history) = peningkatan bertahap di atas seam solver yang sama. Angka PDF bukan acuan.
- **FEM 3D + backend seam + strain-compatibility + buku 230–250/ST1–3** (sesi lanjutan): **#3 Rangka Ruang 3D** `engine/fem/frame3d.ts` (elemen balok-kolom 3D 2-node 6 DOF/node: aksial + torsi GJ + lentur EIy/EIz, lokal 12×12, transformasi 3-sumbu auto up-vector) + tab **🧊** (modeler 3D, isometrik X→kanan/Y→depan/Z→atas + triad, lendutan, gaya N/Vy/Vz/T/My/Mz); divalidasi vs rumus tertutup (5 test). **#4 Backend seam** `engine/fem/backend.ts` (`SolverBackend` interface + `setSolverBackend`/`solve`; frame/frame3d/plate semua lewat seam ini) → backend native Julia/Zig/Zig-C-ABI dapat menggantikan **tanpa ubah kode elemen/UI** (zero-copy Float64Array; toolchain native belum ada di env → didokumentasi, bukan dipalsukan). **Strain-compatibility ULS** `engine/straincompat.ts` (buku 249 Naaman) + tab **🎚**: garis netral c dari regangan riil berlapis, f_ps dari kurva σ–ε tendon aktual, berlaku prategang PENUH & SEBAGIAN, kontrol regangan εt→φ — melengkapi/validasi `uls.ts` aproksimasi. **Buku 230–250 + ST1–3**: mayoritas tercakup — RC T-beam analisis/desain (233/237/238/241/246/248 → `rcgirder.ts`), sectional/MCFT (230 ACI 445, 234/244 Response-2000/Bentz → `mcft.ts`), EC2 jembatan (235 → `ec2.ts`), FEM RC/PC (240 → ekosistem FEM), Suramadu DED (250 → basis gambar spt .dwg), STAAD manual (ST1–3 → validasi modeler) — tidak ada kode redundan; gap nyata = strain-compatibility (249) ditambahkan. Reuse `beta1`/`phiFromStrain` dari substructure.ts. Angka PDF bukan acuan.
- **Ekosistem FEM/FEA (fondasi)** — `engine/fem/` + tab **🧮 FEM Modeler (gaya STAAD.Pro)**. Arsitektur 3-lapis seperti diminta: **Pre-processor** (`fem/model.ts` — geometri/mesh + **3 cara copy/paste**: linearRepeat/translasi, mirror/cermin, rotateCopy/circular; deflectedShape Hermite), **Solver Core** (`fem/core.ts` solver LU pada **Float64Array** = primitif zero-copy JS; `fem/frame.ts` elemen **balok-kolom 2D** 2-node 3-DOF aksial+lentur+geser **Timoshenko bebas shear-locking**; perakitan global, BC penalti, recovery N/V/M), **Post-processor** (bentuk lendutan + diagram N/V/M + reaksi di tab). **Pustaka elemen** juga memuat **flat-shell Q4** (`fem/shell.ts` — membran bilinear + pelat Mindlin **Selective Reduced Integration** → **bebas shear-locking**, dibuktikan test: kurvatur konstan dgn geser γ=0 → energi = lentur murni). Divalidasi vs rumus tertutup (kantilever P·L³/3EI, SS PL³/48EI & 5wL⁴/384EI, aksial PL/EA) + properti elemen shell (12 test FEM). **Keputusan bahasa (jujur):** stack native Python(GUI/pre/post)+Julia(solver)+Zig(memori/sparse/C-ABI zero-copy) = TARGET Phase-2, TAPI toolchain Python/Julia/Zig/gcc TIDAK ADA di environment ini & tak bisa di-deploy ke web app Vercel → diimplementasi di **TypeScript** (jalan di browser, satu-deploy, teruji) dgn seam solver yang sama sehingga backend native CSR+GPU dapat menggantikannya nanti tanpa ubah UI. `📊` diagram gaya dalam kini punya toggle **Sumber: Solver FEM ⇄ Closed-form** (`fem/beamfields.ts` membangun girder jadi model frame, solve, sampel kembali ke bentuk medan yang sama; M_z/V_y dari beban gravitasi + lendutan = gravitasi+camber agar konsisten dgn istilah P·e pada query tegangan; divalidasi ≈ closed-form ≤2%). **Konvensi sumbu global: X→kanan, Y→depan (kedalaman), Z→atas** dgn proyeksi **isometrik** (skala benar, hanya "mojok") + triad sumbu di viewport FEM & Pelat. **Pelat/Shell FEM** (`fem/plate.ts` + tab **▦**): meshing rektangular nx×ny elemen Q4 Mindlin-SRI, tepi SS/clamped, tekanan merata → solve medan w(x,y), permukaan lendutan isometrik berwarna (jet), divalidasi vs teori pelat tipis (rasio FEM/teori ≈1, terbukti tanpa shear-locking termasuk pelat tipis t/a=1/200).
- **Kajian gambar DWG/DXF nyata** (`tools/dwgdump.mjs` + `KAJIAN-GAMBAR-DWG.md`): konverter dipakai SENDIRI untuk membaca **55/55 file `.dwg`** → DXF (tersimpan `dxf_export/`). Dikenali 2 set DED nyata: **Kendilo Bridge** (gelagar baja komposit — plate girder, deck T=250+paving, wing wall, approach slab, pier P1/P2 150T, H-pile 300×300×10×15, sheet pile, pile-head/footing, bearing 100/150Tf fixed/free, `DET-POND` abutment A/B + pier tipe B/C/D + tiang pancang Ø40 + poer) dan **154 Suramadu PCI-Girder** (TAMPAK & POTONGAN, penempatan **DIAFRAGMA tepi & tengah**, strand Ø12,7, diafragma K-350, 526 dimensi). Dimensi nyata 154 (sayap 700/800, badan 200, flens 130–250, b_eff 2100/2150, spasi diafragma 3650/6750/7200, selimut 50/70/80/100) **memvalidasi** preset/default kita. Peningkatan output: `designsheet.ts` Zone B kini menggambar **penempatan diafragma (2 tepi + N tengah, spasi otomatis dari bentang)** pada Tampak Samping — fitur DED menonjol yang sebelumnya belum ada. Angka DWG hanya validasi realisme, BUKAN acuan kode.
- **Buku 219–229 + Impor DXF/DWG** — 219 Xanthakos "Theory & Design of Bridges", 220 Bridge Engineering Handbook (Chen/Duan), 221 Construction & Maintenance, **222/224 Seismic Design**, **223 Substructure Design**, 225/227/228 Bridge Maintenance/Safety/Life-Cycle, 226 Earthquake Engineering for Structural Design, 229 Computational Analysis & Design of Bridge Structures (Fu & Wang). Semua MEMVALIDASI modul yang sudah ada (seismic spektrum & dinamik → `sni2833seismic.ts`/`seismicdynamics.ts`, substruktur → `substructure.ts`, fondasi dinamik/SSI → `foundationdynamics.ts`) — prosedur sudah tercakup, **tidak ada kode redundan**. **Pembacaan gambar CAD:** `.dwg` biner (AC1015/AC1018) TIDAK dapat diparse di lingkungan ini (tak ada konverter ODA/AutoCAD/python). Solusi = **DXF importer** (`engine/dxfimport.ts` parser DXF ASCII murni + tab **📐 Impor Gambar DXF**): ekspor DWG→DXF di CAD (SAVE AS → DXF / DXFOUT), unggah → parser mengekstrak extents (panjang/lebar jembatan), bounding-box profil girder, spasi girder/diafragma (median garis vertikal), nilai DIMENSION (kode 42), TEXT/MTEXT, & kotak substruktur (abutment/pier/pilecap/pierhead) → tombol "terapkan" mengisi bentang/lebar dek/tinggi girder ke store (selektor satuan mm/cm/m). Nilai tinggi abutment/pier, frontwall, wingwall, pile dibaca dari daftar teks/dimensi lalu diisikan ke tab 🏛️/🪨.
- **Analisis dinamik & desain gempa BANGUNAN BAWAH** (AASHTO Guide Spec / Caltrans SDC / SNI 2833 / Seed–Idriss — divalidasi buku 222/223/224/226; karena aturan proyek hanya memakai prosedur/alur, modul diimplementasi dari standar baku: AASHTO Guide Spec for LRFD Seismic Bridge Design, Caltrans SDC, Priestley/Calvi/Kowalsky DBD, SNI 2833, Seed–Idriss/Youd likuifaksi). Engine baru `engine/seismicdynamics.ts` (BEDA dari seismic.ts mode-tunggal, sni2833seismic.ts spektrum, foundationdynamics.ts mesin/SSI): `computeSDOF` (pilar sbg osilator: T=2π√(m/K), faktor redaman B, Sd=Sa·g/ω², V_base), `computeModal2` (2-DOF eigen det(K−ω²M)=0, mode shape, partisipasi Γ, kombinasi SRSS), `computeCapacityDesign` (sendi plastis: M_po=λ_o·M_p overstrength, V_po geser kapasitas, L_p Priestley, Δ_y, Δ_p, Δ_C, daktilitas μ_Δ, cek P-Δ ≤0,25M_p), `computeLiquefaction` (Seed–Idriss/Youd: CSR=0,65(a_max)(σ_v/σ′_v)r_d, (N₁)₆₀cs koreksi fines, CRR₇,₅, MSF, FS). Tab baru **🌋 Dinamik & Gempa Bangunan Bawah** (4 sub-tab default-filled). **Terhubung ke centang Pondasi**: bila `foundation.enabled` dicentang, runPipeline juga menghitung blok seismik (K_pilar=3EI/H³ → SDOF Sd → demand Δ_D desain kapasitas, likuifaksi dari data tanah pondasi) → **§31 di laporan PDF** (format 3-baris + cek Δ_D≤Δ_C, P-Δ, FS likuifaksi). FoundationConfig diperluas (seismicSa/pierMp/pierH/pierD/amax/Mw/N160/fines), input di InputPanel.
- **Buku 194–218 + semua `*.dwg`** — analisis & desain PONDASI (statik + dinamik) dan PEMBEBANAN/GEMPA/STRUKTUR Indonesia. Sumber: 194 Bowles, 195 Budhu, 198 Curtin, 201 US Army TM 5-818-1, 204 French (fondasi statik); 200 Vulcanhammer wave-equation/pemancangan; 196/199/202/203 Das + 205 (dinamik fondasi/mesin, pegas-redaman/impedansi tanah, SSI); 207 SNI 1725:2016, 211 SNI 2833:2016, 209 Gimsing cable-supported, 210 "Desain Jembatan Rangka Baja" (Rochman & Suhariyanto), 215 AASHTO LRFD SI 2007, 214/216/217 manual DOT (tercakup). Lima modul engine baru/diperluas (angka PDF BUKAN acuan — hanya prosedur): `engine/pilefoundation.ts` (kapasitas aksial α/β/Meyerhof Q_s+Q_p, grup Converse-Labarre + blok, penurunan Vesic, lateral Broms, pemancangan dinamik ENR/ModENR/Hiley/Janbu), `engine/foundationdynamics.ts` (daya dukung dangkal Vesic N_c/N_q/N_γ, fondasi mesin half-space Richart k/D/f_n/amplitudo 4 mode + SSI Veletsos T̃/T), `engine/sni2833seismic.ts` (spektrum respons As/S_DS/S_D1/T0/Ts/C_sm, zona SDC, faktor R — BEDA dari seismic.ts mode-tunggal), `engine/cablestayed.ts` (Gimsing — stay fan/harp, gaya V/sinθ, Ernst E_eff, aksial pilon), `engine/steeltruss.ts` (rangka Pratt/Warren/Howe — chord M/h, diagonal V/sinθ, tarik leleh + tekan tekuk F_cr), + `bridgeload.ts` `computeSecondaryLoads` (SNI 1725 angin EWs/EWl, rem TB, suhu EUn). UI: 4 tab baru di ExtraCalculators (🪨 Pondasi, 🌎 Beban & Gempa SNI, 🪢 Jembatan Kabel, 🔺 Jembatan Rangka Baja) — terisi default + diagram SVG. **Centang opt-in**: `foundation.enabled` di InputPanel → pipeline menghitung §30 Pondasi & MUNCUL di laporan PDF (format 3-baris); jika tak dicentang, tidak ikut dihitung/dimunculkan. 218.pdf & `*.dwg` = scan/biner (tak terekstrak) → dipakai sebagai basis gaya gambar saja.
- **Buku 181–193 (+170.pdf)** — mayoritas tercakup: 181 WSDOT §6-02 handling/penyimpanan + "D" dimension batas camber (→ `handling.ts`/`lateralstability.ts`/`aemm.ts`), 182 PCI BDM Ch.6 desain pendahuluan (→ `computePrelimPT`), 183 AASHTO Box Beam dims (→ `AASHTO_BOX`), 184 Hassanain adjacent-box LRFD optimization (→ `optimization.ts`/`transversept.ts`), 185 Double-Tee (→ `DOUBLE_T`), 186 uji geser girder 40-thn quarter-point rule + 189 FHWA LWHPC shear (→ validasi `mcft.ts`, no kode baru), 188 FDOT Index 20120 AASHTO Type II, 190 Rabbat-Russell optimized sections (= asal PCI Bulb-Tee, sudah ada), 193 studi kasus PCI girder Padang Pariaman. SATU famili produk standar yang belum ada → **AASHTO Solid & Voided Slab Beam SI–SIV** (buku 192, kategori baru `AASHTO_SLAB`, 36/48 in × H 12/15/18/21 in, solid + voided, idealisasi 1-web spt VOIDED_SLAB). DB profil kini **70 profil / 14 kategori**. Strand 0.5″/0.6″ (G1725/G1860) + multi-strand PT sudah lengkap di `lib/strands.ts`. Tetap: angka PDF bukan acuan kode — hanya bab/sub-bab/urutan/prosedur/kelengkapan; tidak menebak profil ambigu agar tak redundant.
