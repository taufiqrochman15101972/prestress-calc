...
const TABS: { key: ExtraTab; emoji: string; title: string; subtitle: string }[] = [
  ...
  },
  {
    key: "modal",
    emoji: "📳",
    title: "Analisis Modal & Spektrum Respons",
    subtitle: "Analisis modal N-DOF umum & Response Spectrum Analysis: eigenproblem K φ=ω²M φ (reduksi Cholesky + Jacobi siklik) → perioda T_n, ragam getar, faktor partisipasi Γ_n, massa modal efektif M_n* & rasio massa kumulatif (≥90% ASCE §12.9); RSA multi-modus u=Γφ·Sa/ω² & gaya=ΓMφ·Sa, kombinasi SRSS & CQC (korelasi Der Kiureghian/Wilson). Pakai spektrum ASCE 7-16 (sinergi tab 🏙️). Melengkapi time-history SDOF 🌊 & modal 2-DOF jembatan 🌋 (books DS 1–96)",
  },
  {
    key: "forcemethod",
    emoji: "🔢",
    title: "Metode Matriks Gaya & Tiga-Momen",
    subtitle: "Metode GAYA (fleksibilitas) analisis struktur matriks — dual dari metode kekakuan FEM: persamaan Tiga-Momen Clapeyron untuk balok menerus N-bentang (sistem tridiagonal momen tumpuan → reaksi & momen lapangan), redundan via matriks fleksibilitas [f]{X}=−{Δ0}, kasus klasik (kantilever-prop R=3wL/8, jepit-jepit M=wL²/12, menerus 2-bentang M=wL²/8 & R=1,25wL). Verifikasi silang FEM 🧮. Pustaka MTH Przemieniecki/Azar/Paz Matrix Structural Analysis (MTH 1–116)",
  },
  {
    key: "snieq",
    emoji: "🌎",
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-10">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl z-10 border border-gray-200
        ...
        <div className="px-4 py-1.5 bg-blue-50 border-b border-blue-100 text-[10px] text-blue-700">
          ...
          {tab === "modal" && "Pustaka DS 1–96 (dinamika struktur & rekayasa gempa): Chopra 'Dynamics of Structures', Craig & Kurdila 'Fundamentals of Structural Dynamics', Gupta 'Response Spectrum Method', Wilson '3-D Static & Dynamic Analysis' (penulis CQC), Paz, Humar, Clough & Penzien, Anderson & Naeim. ANALISIS MODAL N-DOF UMUM & RESPONSE SPECTRUM ANALYSIS (melengkapi time-history SDOF 🌊 linier & modal 2-DOF jembatan 🌋): (1) eigenproblem tergeneralisasi K φ=ω²M φ — reduksi Cholesky M=LLᵀ → Ã=L⁻¹KL⁻ᵀ → Jacobi siklik simetris → φ=L⁻ᵀψ (ragam ter-normalisasi-massa φᵀMφ=1); (2) perakit bangunan-geser massa-gumpal (M diagonal, K tridiagonal); (3) properti modal: ω_n/T_n/f_n, faktor partisipasi Γ_n=φᵀMr/φᵀMφ, massa modal efektif M_n*=(φᵀMr)²/φᵀMφ (Σ=massa total), rasio kumulatif ≥90% (ASCE §12.9); (4) RSA multi-modus u_n=Γ_n φ_n·Sa(T_n)/ω_n², gaya lantai f_n=Γ_n Mφ_n·Sa(T_n), kombinasi SRSS & CQC (ρᵢⱼ=8ζ²(1+r)r^1,5/[(1−r²)²+4ζ²r(1+r)²]). Spektrum desain ASCE 7-16 dipakai bersama 🏙️. Angka contoh PDF bukan acuan — hanya rumus/prosedur"}
          {tab === "forcemethod" && "Pustaka MTH 1–116 (analisis struktur matriks & metode numerik): Przemieniecki 'Theory of Matrix Structural Analysis', Azar 'Matrix Structural Analysis', Mario Paz 'Integrated Matrix Analysis of Structures', Weaver & Gere, Hartmann, McGuire/Gallagher; + Maple/MATLAB/Mathcad/numerical methods. Metode kekakuan (perpindahan) SUDAH jadi fondasi FEM 🧮/🧊/▦/▣ + eigen modal 📳. Tambahan = METODE GAYA (fleksibilitas) — dual klasik yang belum ada: (1) persamaan TIGA-MOMEN Clapeyron balok menerus N-bentang — rakit sistem tridiagonal momen tumpuan M_{n−1}L_n+2M_n(L_n+L_{n+1})+M_{n+1}L_{n+1}=−¼(w_n L_n³+w_{n+1}L_{n+1}³) → momen tumpuan → reaksi & momen lapangan; (2) inti metode gaya umum: matriks fleksibilitas [f]{X}=−{Δ0} → redundan; (3) kasus tertutup klasik kantilever-prop (R=3wL/8, M=wL²/8), jepit-jepit (M=wL²/12), menerus 2-bentang sama (M_B=wL²/8, R_B=1,25wL). VERIFIKASI SILANG hasil metode kekakuan FEM 🧮 lewat formulasi gaya. Angka contoh PDF bukan acuan — hanya rumus/prosedur"}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          {tab === "modal" && <ModalDynamicsCalculator />}
          {tab === "forcemethod" && <ForceMethodCalculator />}
